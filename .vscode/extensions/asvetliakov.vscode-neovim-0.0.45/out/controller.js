...
function convertByteNumToCharNum(line, col, multiBytes2ByteWidth = false) {
    if (col === 0 || !line) {
        return 0;
    }
    let currCharNum = 0;
...
function getStartColForHL(line, byteCol) {
    if (!line) {
        return 0;
    }
    let currByteCol = 0;
    ...
    constructor(neovimPath, extensionPath, highlightsConfiguration, mouseSelection, useWsl, customInit = "") {
                                ...
                                if (!this.isInsertMode) {
                                    ...
                                    if (editor.viewColumn) {
                                        ...
                                        if (winId) {
                                            ...
                                            if (gridConf) {
                                                editor.selections = [
                                                    new vscode_1.default.Selection(gridConf[1].cursorLine, gridConf[1].cursorPos, gridConf[1].cursorLine, gridConf[1].cursorPos),
                                                ];
                                            }
                                        }
            ...
            for (const [name, ...args] of batch) {
                ...
                switch (name) {
                    ...
                    case "grid_line": {
                        ...
                        const lastLinesEvents = gridEvents.filter(([grid, line, , cells]) => ((grid === 2 && line === LAST_SCREEN_LINE_FIRST_GRID) ||
                        ...
                        for (const evt of firstLinesEvents) {
                            ...
                            const topLineStr = processLineNumberStringFromEvent(evt, this.numberLineHlId, gridConf.topScreenLineStr);
                        ...
                        for (const evt of lastLinesEvents) {
                            ...
                            const bottomLineStr = processLineNumberStringFromEvent(evt, this.numberLineHlId, gridConf.bottomScreenLineStr);
        ...
        this.applyRedrawUpdate = (newModeName, cursorUpdates, hlUpdates, acceptPrompt) => {
            ...
            for (const grid of hlUpdates) {
                ...
                const hls = this.highlightProvider.getGridHighlights(grid, getLineFromLineNumberString(gridConf.topScreenLineStr));
        ...
        this.resyncCursor = () => __awaiter(this, void 0, void 0, function* () {
            const e = vscode_1.default.window.activeTextEditor;
            if (!e || !e.viewColumn) {
                return;
            }
            ...
            const [line1based, col0based] = yield this.client.callFunction("nvim_win_get_cursor", [winId]);
    ...
    applyTextDecorations(hlGroupName, decorations) {
        ...
        for (const [lineStr, cols] of decorations) {
            ...
            for (const [colNum, text] of cols) {
                const col = colNum - 1;
                const opt = {
                    range: new vscode_1.default.Range(line, col, line, col),
                    renderOptions: {
        ...
        if (vscode_1.default.window.activeTextEditor) {
            vscode_1.default.window.activeTextEditor.setDecorations(decorator, options);
        }
    }
